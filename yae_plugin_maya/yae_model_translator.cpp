...
MStatus  yae_model_translator::reader(const MFileObject &file, const MString &optionsString, FileAccessMode mode)
{
	yae::file_system fs;
	yae::mesh m;
	...
	res = m.load( file.expandedFullName().asChar() );
	if( !res ) {
...
MStatus yae_model_translator::create_shape( const yae::mesh &m )
{
	MFloatPointArray v;
	MVectorArray norm;
	MIntArray p;
	...
	yae::mesh::vertices mv = m.get_vertices();
	yae::mesh::indices mi = m.get_indices();
	yae::mesh::texcoords mt = m.get_tex_coords();
...
MObject yae_model_translator::create_material( const std::string texture, MStatus *status )
{
	MString tex_name;
...
MPxFileTranslator::MFileKind yae_model_translator::identifyFile(const MFileObject &file, const char *buffer, short size) const
{
	MString name = file.name();
	MString ext = name.substring( name.rindex( '.' ), name.length() ).toLowerCase();
	if( ext != MString( ".ds2md" ) )
